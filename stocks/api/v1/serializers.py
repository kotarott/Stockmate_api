...
class CommentSerializer(serializers.ModelSerializer):
    author = UnitProfileSerializer(read_only=True)
    symbols_obj = serializers.SerializerMethodField()
    reply_obj = ParentCommentSerializer(source='reply',read_only=True)
    likes_count = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        extra_kwargs = {
            'symbols': {'write_only': True},
            'reply': {'write_only': True},
        }

    def get_symbols_obj(self, instance):
    ...
    def get_replys_count(self, instance):
        return instance.reply_comment.count()
...
class FMPSymbolProfileSerializer(serializers.Serializer):
    ...
    currency = serializers.CharField(max_length=3)
    mktCap =serializers.IntegerField()
    ...
    image_db = serializers.SerializerMethodField()
    photo = serializers.SerializerMethodField()
    user_has_liked_symbol = serializers.SerializerMethodField()
    likes_count = serializers.SerializerMethodField()
    vender = serializers.SerializerMethodField()
    ...
    def get_image_db(self, instance):
        request = self.context.get('request')
        symbol = Symbol.objects.filter(symbol=instance['symbol'])
        if symbol.exists():
            if symbol.get().image:
